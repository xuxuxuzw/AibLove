...
class Oauth extends Controller
{
	...
	private function _get_login_redirect()
    {
		...
		return empty($login_http_referer_s)?WEB_ROOT."/":$login_http_referer_s;
	}
	...
	private function _bang_handle($user_info, $type, $token)
    {
		...
		$oauth_id=($type=='wechat' || $type=='weixin')?'openid':'openid';
		$find_oauth_user = Db::table('oauth_user')->where(array("oauth_from"=>$type,$oauth_id=>$token[$oauth_id]))->find();
		$need_bang=true;


			dump($find_oauth_user);
			dump($user_info);
			dump($type);
			dump($token);
			exit();
		...
		if($need_bang){
			if($current_uid){
				...
				$new_oauth_user_id=Db::table('oauth_user')->insertGetId($new_oauth_user_data);
	...
	private function _login_handle($user_info, $type, $token)
    {
		...
		if($find_oauth_user){
			...
			Db::table('oauth_user')->where(array("oauth_from"=>$type,$oauth_id=>$token[$oauth_id]))->update($data);
			$find_user = Db::table("member_list")->where(array("member_list_id"=>$find_oauth_user['uid']))->find();
			if($find_user){
				$need_register=false;
				...
				Db::table("member_list")->where(array('member_list_id'=>$find_user["member_list_id"]))->update($data);
				session('hid',$find_user['member_list_id']);
				session('user',$find_user);
				
				//根据需要决定是否同步后台登录状态
				$admin=Db::table('admin')->where('member_id',$find_user["member_list_id"])->find();
                if($admin){
                    // 记录登录
                    $auth = array(
                        'aid'             			 => $admin['admin_id'],
		...
		if($need_register){
			...
			$users_model=Db::table("member_list");
			$new_user_id = Db::table("member_list")->insertGetId($new_user_data);
			$new_user_data=Db::table("member_list")->find($new_user_id);
			if($new_user_id){
				//第三方用户表中创建数据
				$new_oauth_user_data = array(
					'oauth_from' => $type,
